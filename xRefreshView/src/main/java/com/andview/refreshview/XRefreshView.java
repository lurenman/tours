...
public class XRefreshView extends LinearLayout
{
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev)
    {
        switch (ev.getAction())
        {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                else if (!mPullRefreshing && mContentView.isBottom() && (deltaY < 0 || deltaY > 0 && mHolder.hasFooterPullUp()))
                {
                    return true;
                }
                return false;
    ...
    public boolean onTouchEvent(MotionEvent ev)
    {
        ...
        {
            sendCancelEvent();
            updateHeaderHeight((int) ev.getRawY(), deltaY);
            return true;
        }
        else if (!mPullRefreshing && mContentView.isBottom() && (deltaY < 0 || deltaY > 0 && mHolder.hasFooterPullUp()))
        {
        ...
        else if (deltaY != 0 && (mContentView.isTop() && !mHolder.hasHeaderPullDown() || mContentView.isBottom() && !mHolder.hasFooterPullUp()))
        {
    ...
    public void stopLoadMore(final boolean hideFooter)
    {
        if (needAddFooterView())
        {
            if (mPullLoading == true)
            {
                ...
                if (mPinnedTime >= 1000)
                {// 在加载更多完成以后，只有mPinnedTime大于1s才生效，不然效果不好
                    postDelayed(new Runnable()
                    {

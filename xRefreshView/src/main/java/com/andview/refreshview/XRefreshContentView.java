...
public class XRefreshContentView implements OnScrollListener, OnTopRefreshTime, OnBottomLoadMoreTime
{
    ...
    {
        if (mFooterCallBack == null || !isFooterEnable())
            return;
        final RecyclerView recyclerView = (RecyclerView) child;
        if (hasComplete)
        {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState)
    {
        ...
        else if (mContainer != null && !hasLoadCompleted() && scrollState == OnScrollListener.SCROLL_STATE_IDLE)
        {
            if (mPreLoadCount == 0)
            {
                if (isBottom())
    ...
    public boolean canChildPullDown()
    {
        if (child instanceof AbsListView)
        {
            ...
            return canScrollVertically(child, -1) || absListView.getChildCount() > 0 && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(0).getTop() < absListView.getPaddingTop());
        }
        else
        {
    ...
    public boolean canChildPullUp()
    {
        ...
        {
            AbsListView absListView = (AbsListView) child;
            return canScrollVertically(child, 1) || absListView.getLastVisiblePosition() != mTotalItemCount - 1;
        }
        else if (child instanceof WebView)
        {
            WebView webview = (WebView) child;
            return canScrollVertically(child, 1) || webview.getContentHeight() * webview.getScale() != webview.getHeight() + webview.getScrollY();
        }
        else if (child instanceof ScrollView)
        {
            ...
            if (childView != null)
            {
                return canScrollVertically(child, 1) || scrollView.getScrollY() < childView.getHeight() - scrollView.getHeight();
            }
        }
        else
        {
